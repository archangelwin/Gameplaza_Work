...
bool CServiceUnits::InitializeService()
{
	...
	if (m_TCPNetworkEngine->SetTCPNetworkEngineEvent(pIAttemperEngine,m_GameServiceOption.szServerName)==false) return false;
...
bool CServiceUnits::LoadDataBaseParameter(LPCTSTR pszDataBaseAddr, LPCTSTR pszDataBaseName, tagDataBaseParameter & DataBaseParameter)
{
	...
	m_PlatformDBModule->SetConnectionInfo(pPlatformDBParameter->szDataBaseAddr,pPlatformDBParameter->wDataBasePort,
		...
		m_GameServiceOption.szServerName,"PlatformDBModule_LoadDataBaseParameter");
	...
	try
	{
		...
		m_PlatformDBAide.AddParameter(TEXT("@strDataBaseAddr"),pszDataBaseAddr);

		//执行查询
		if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GS_LoadDataBaseInfo"),true)!=DB_SUCCESS)
		{
			//构造信息
		...
		//获取信息
		DataBaseParameter.wDataBasePort=m_PlatformDBAide.GetValue_WORD(TEXT("DBPort"));
	...
	catch (IDataBaseException * pIException)
	{
		...
		CTraceService::TraceString(TEXT("GSP_GS_LoadDataBaseInfo"),TraceLevel_Exception);
