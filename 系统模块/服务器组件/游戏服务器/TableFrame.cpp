...
//房间消息
bool CTableFrame::SendRoomMessage(LPCTSTR lpszMessage, WORD wType)
{
	//发送消息
	m_pIMainServiceFrame->SendRoomMessage(lpszMessage,wType);
...
const tagStockInfo * CTableFrame::GetStockInfo(int nRoom, int nTable, int nType)
{
	return m_pIMainServiceFrame->GetStockInfo(m_wTableID, nType);
}

void CTableFrame::ChangeStock(int nType, WORD wChairID, int nStock, int nRevenue, int nCaiJin)
{
	unsigned int nUserID = 0;

	m_pIMainServiceFrame->ChangeStock(nType, m_wTableID, nUserID, nStock, nRevenue, nCaiJin);
}

float CTableFrame::GetUserFactor(unsigned int nUserID)
{
	return m_pIMainServiceFrame->GetUserFactor(nUserID);
}

int CTableFrame::SaveUserItem(IServerUserItem * pIServerUserItem, int nItemIndex)
{
	return m_pIMainServiceFrame->SaveUserItem(pIServerUserItem, nItemIndex);
}

//游戏记录Json格式
void CTableFrame::SaveGameJson(IServerUserItem * pIServerUserItem, const TCHAR * str, SCORE lScore)
{
	if (NULL==pIServerUserItem || NULL==str) return;
	m_pIMainServiceFrame->SaveGameJson(pIServerUserItem,str,lScore);
}

//获取游戏内部分数
LONG CTableFrame::GetInsideScore(WORD wChairID)
{
	if (NULL==m_pITableFrameSink) return 0;
	return m_pITableFrameSink->GetInsideScore(wChairID);
}

tagLotteryInfo * CTableFrame::GetLotteryInfo()
{
	return m_pIMainServiceFrame->GetLotteryInfo();
}

tagMobileMoney * CTableFrame::GetMobileMoney()
{
	return m_pIMainServiceFrame->GetMobileMoney();
}

void CTableFrame::getRecordCount()
{
}

//赢得龙珠碎片
bool CTableFrame::WinDragonPiece(IServerUserItem * pIServerUserItem, SCORE lWinDragonPiece, SCORE lWinScore)
{
	...
	return m_pIMainServiceFrame->WinDragonPiece(pIServerUserItem, lWinDragonPiece, lWinScore);
}

//赢得珍珠
bool CTableFrame::WinPearl(IServerUserItem * pIServerUserItem, int nPearlType, int nPearlCount)
{
	...
	return m_pIMainServiceFrame->WinPearl(pIServerUserItem, nPearlType, nPearlCount);
}

//系统赠送技能
bool CTableFrame::SkillGive(IServerUserItem * pIServerUserItem)
{
	if (pIServerUserItem == NULL)
	{
		return false;
	...
	return m_pIMainServiceFrame->SkillGive(pIServerUserItem);
}

//新手引导奖励
bool CTableFrame::NewGuidGift(IServerUserItem * pIServerUserItem)
{
	if (pIServerUserItem == NULL)
	{
		return false;
	...
	return m_pIMainServiceFrame->OnNewGuidGift(pIServerUserItem);
}

//新手引导奖励
bool CTableFrame::OnNewGuidGift(IServerUserItem * pIServerUserItem, bool bSuccess, int nCannonStatus, int nLotteryCount)
{
	return m_pITableFrameSink->OnNewGuidGift(pIServerUserItem, bSuccess, nCannonStatus, nLotteryCount);
}

//赢得珍珠
bool CTableFrame::OnWinPearl(IServerUserItem * pIServerUserItem, int nPearlType, int nPearlCount)
{
...
{
	return m_pIMainServiceFrame->SendMatchScore(pIServerUserItem, lMatchScore);
}

//处理比赛排名
void CTableFrame::OnMatchRank(IServerUserItem * pIServerUserItem, SCORE lMatchScore, int nCurrentRank, int nChangeRank)
{
	m_pITableFrameSink->OnMatchRank(pIServerUserItem, lMatchScore, nCurrentRank, nChangeRank);
}

//发送全服广播
void CTableFrame::SendPlatformMessage(LPCTSTR lpszMessage, BYTE cbLevel)
{
	m_pIMainServiceFrame->SendPlatformMessage(lpszMessage, cbLevel);
}

//获取用户道具
void CTableFrame::GetUserItemCount(IServerUserItem * pIServerUserItem, int nType)
{
printf("%s %d\n", __FUNCTION__, __LINE__);
	...
	m_pIMainServiceFrame->GetUserItemCount(pIServerUserItem, nType);
}

//保存用户道具
void CTableFrame::SaveUserItemCount(IServerUserItem * pIServerUserItem, int nType, SCORE lCount)
{
printf("%s %d\n", __FUNCTION__, __LINE__);
	if (pIServerUserItem == NULL)
	{
	...
	m_pIMainServiceFrame->SaveUserItemCount(pIServerUserItem, nType, lCount);
}

//获取用户道具
bool CTableFrame::OnGetUserItemCount(IServerUserItem * pIServerUserItem, int nType, SCORE lCount)
{
printf("%s %d\n", __FUNCTION__, __LINE__);
	return m_pITableFrameSink->OnGetUserItemCount(pIServerUserItem, nType, lCount);
}

void CTableFrame::SetTreasureMapCount(IServerUserItem * pIServerUserItem,int nCount)
{
	if (pIServerUserItem == NULL)
	{
		printf("%s pIServerUserItem=%p\n", __FUNCTION__, pIServerUserItem);
		return ;
	}
	printf("CtableFrame=%d\n",nCount);
	return m_pIMainServiceFrame->SetTreasureMapCount(pIServerUserItem,nCount);
}

void CTableFrame::UpdateUserKValueAndIntegration(IServerUserItem * pIServerUserItem,int nKvalue,int nIntegration)
{
	if (pIServerUserItem == NULL)
	{
		printf("%s pIServerUserItem=%p\n", __FUNCTION__, pIServerUserItem);
	...
	return m_pIMainServiceFrame->UpdateUserKValueAndIntegration(pIServerUserItem,nKvalue,nIntegration);
}

void CTableFrame::UpdateUserMaxKValue(IServerUserItem * pIServerUserItem,int nCount)
{
	if (pIServerUserItem == NULL)
	{
		printf("%s pIServerUserItem=%p\n", __FUNCTION__, pIServerUserItem);
	...
	printf("CtableFrame=%d\n",nCount);
	return m_pIMainServiceFrame->UpdateUserMaxKValue(pIServerUserItem,nCount);
}

void CTableFrame::UpdateUserIntegrate(IServerUserItem * pIServerUserItem,int nAddScore)
{
	if (pIServerUserItem == NULL)
	{
		printf("%s pIServerUserItem=%p\n", __FUNCTION__, pIServerUserItem);
	...
	return m_pIMainServiceFrame->UpdateUserIntegrate(pIServerUserItem,nAddScore);
}

bool CTableFrame::OnWinBigReward(IServerUserItem * pIServerUserItem,int nScore)
{
	if (pIServerUserItem == NULL)
	{
		printf("%s pIServerUserItem=%p\n", __FUNCTION__, pIServerUserItem);
