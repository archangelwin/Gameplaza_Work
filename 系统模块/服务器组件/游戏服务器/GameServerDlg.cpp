...
VOID CGameServerDlg::UpdateServerTitle(enServiceStatus ServiceStatus)
{
	...
	_sntprintf(szTitle,CountArray(szTitle),TEXT("[%s]--[%s]--[roomid=%d][pid=%d]"),pszServerName,pszStatusName,m_ModuleInitParameter.GameServiceOption.wServerID,_getpid());
...
VOID CGameServerDlg::UpdateParameterStatus(tagModuleInitParameter & ModuleInitParameter)
{
	...
	CString sServerName=TEXT("");
	sServerName.Format(TEXT("%s(%s)"),m_ModuleInitParameter.GameServiceOption.szServerName,szPath);

	//设置控件
	SetDlgItemText(IDC_GAME_NAME,m_ModuleInitParameter.GameServiceAttrib.szGameName);
	SetDlgItemText(IDC_SERVER_NAME,sServerName);
...
{
	m_ServiceUnits.m_AttemperEngineSink.ShowRealOnlineCount();
}

//写分并踢出
VOID CGameServerDlg::OnBnClickedServiceControl()
{
	LPCTSTR pszQuestion=TEXT("游戏服务器正在运行中，您确实要踢出所有玩家吗？");
	if (AfxMessageBox(pszQuestion,MB_YESNO|MB_DEFBUTTON2|MB_ICONQUESTION)!=IDYES) return;
	m_ServiceUnits.m_AttemperEngineSink.SaveAndKick();
}
