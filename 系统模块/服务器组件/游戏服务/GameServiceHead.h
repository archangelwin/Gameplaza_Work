...
struct tagStockInfo
{
	int mStockScore;  //库存分数
	int mRevenue;	  //抽水分数
	int mCaiJin;	  //彩金分数
	int mCaiJinSent;  //彩金积累
	int mRoomID;	  //房间ID -1为所有房间
	int mTableID;	  //桌子ID -1为所有桌子
	int mType;
...
//@Add
struct tagMobileMoney
{
	bool bCouldGet;//可以获得
	WORD wPrice;//话费金额
	WORD wMaxGet;//单个玩家最多获得数量
	int nLessTime;//至少玩多少秒才开始获得
	int nPercent;//获得概率
	int nMinusStorage;//从库存中减掉的分数
};
...
public:
	...
	//设置炮台状态
	virtual void SetCannonStatus(int nCannonStatus)=NULL;
	//获取炮台状态
	virtual int GetCannonStatus()=NULL;
	virtual void SetPearl(int nType)=NULL;
	virtual int GetPearl()=NULL;
	virtual void SetVipInfo(BYTE cbVIPLevel, DWORD dwVIPExp)=NULL;
	virtual BYTE GetVipLevel()=NULL;
...
public:
	...
	//比赛积分
	virtual SCORE GetMatchScore()=NULL;
	//比赛积分
	virtual void SetMatchScore(SCORE s)=NULL;
...
public:
	...
	//获取比赛报名状态
	virtual BYTE GetUserEnlist()=NULL;
	//设定比赛报名状态
	virtual VOID SetUserEnlist(BYTE cbEnlistStatus)=NULL;
...
public:
	...
	//用户操作日志
	virtual bool WriteUserLog(WORD wRequestID, VOID  * pData, DWORD dwDataSize)=NULL;
...
public:
	...
	//用户信息
	virtual tagUserInfo * GetUserData() = NULL;
	//用户物品
	virtual bool AddUserItem(const tagUserItem * pUserItem) = NULL;
	virtual tagUserItem * GetUserItems() = NULL;
	virtual int GetUserItemCount() = NULL;
	virtual void SetTreasureMapCount(int nCount) = NULL;
	virtual int GetTreasureMapCount() = NULL;
	virtual void UpdateUserKValueAndIntegration(int nKvalue,int nIntegration) = NULL;
	virtual int GetUserKValue() = NULL;
	virtual void UpdateUserMaxKValue(int nCount) = NULL;
	virtual int GetUserMaxKValue() = NULL;
};
...
interface IServerUserItemSink : public IUnknownEx
{
	...
	//用户日志
	virtual bool OnEventUserItemLog(IServerUserItem *pIServerUserItem, WORD wRequestID, VOID * pData, DWORD dwDataSize)=NULL;
};
...
public:
	virtual const tagStockInfo * GetStockInfo(int nRoom, int nTable, int nType)=NULL;
	virtual void ChangeStock(int nType, WORD wChairID, int nStock, int nRevenue, int nCaiJin)=NULL;
	virtual float GetUserFactor(unsigned int nUserID)=NULL;
	virtual int SaveUserItem(IServerUserItem * pIServerUserItem, int nItemIndex)=NULL;
	//游戏记录Json格式
	virtual void SaveGameJson(IServerUserItem * pIServerUserItem, const TCHAR * str, SCORE lScore)=NULL;
	//获取游戏内部分数
	virtual	LONG GetInsideScore(WORD wChairID)=NULL;
	virtual tagLotteryInfo * GetLotteryInfo()=NULL;
	virtual tagMobileMoney * GetMobileMoney()=NULL;
	//赢得龙珠碎片
	virtual bool WinDragonPiece(IServerUserItem * pIServerUserItem, SCORE lWinDragonPiece, SCORE lWinScore)=NULL;
	//赢得珍珠
	virtual bool WinPearl(IServerUserItem * pIServerUserItem, int nPearlType, int nPearlCount)=NULL;
	//系统赠送技能
	virtual bool SkillGive(IServerUserItem * pIServerUserItem)=NULL;
	//新手引导奖励
	virtual bool NewGuidGift(IServerUserItem * pIServerUserItem)=NULL;
	//发送比赛积分
	virtual bool SendMatchScore(IServerUserItem * pIServerUserItem, SCORE lMatchScore)=NULL;
	//处理比赛排名
	virtual void OnMatchRank(IServerUserItem * pIServerUserItem, SCORE lMatchScore, int nCurrentRank, int nChangeRank)=NULL;
	//发送全服广播
	virtual void SendPlatformMessage(LPCTSTR lpszMessage, BYTE cbLevel)=NULL;
	//获取用户道具
	virtual void GetUserItemCount(IServerUserItem * pIServerUserItem, int nType)=NULL;
	//保存用户道具
...
public:
	...
	//新手引导奖励
	virtual bool OnNewGuidGift(IServerUserItem * pIServerUserItem, bool bSuccess, int nCannonStatus, int nLotteryCount)=NULL;
	//赢得珍珠
	virtual bool OnWinPearl(IServerUserItem * pIServerUserItem, int nPearlType, int nPearlCount)=NULL;
	//获取用户道具
...
public:
	...
	virtual const tagStockInfo * GetStockInfo(WORD nTable, int nType)=NULL;
	virtual void ChangeStock(int nType, WORD wTableID, DWORD nUserID, int nStock, int nRevenue, int nCaiJin)=NULL;
	virtual float GetUserFactor(unsigned int nUserID)=NULL;
	virtual int SaveUserItem(IServerUserItem * pIServerUserItem, int nItemIndex)=NULL;
	//游戏记录Json格式
	virtual void SaveGameJson(IServerUserItem * pIServerUserItem, const TCHAR * str, SCORE lScore)=NULL;
	...
	virtual bool SkillGive(IServerUserItem * pIServerUserItem)=NULL;
	//新手引导奖励
	virtual bool OnNewGuidGift(IServerUserItem * pIServerUserItem)=NULL;
	//发送比赛积分
	virtual bool SendMatchScore(IServerUserItem * pIServerUserItem, SCORE lMatchScore)=NULL;
	//发送全服广播
	virtual void SendPlatformMessage(LPCTSTR lpszMessage, BYTE cbLevel)=NULL;
...
public:
	...
	virtual bool  InitMatchInterface(ITCPNetworkEngineEvent *pTCPNetworkEngine,IDataBaseEngine *pIDataBaseEngine, IServerUserManager *pIServerUserManager,
					IMainServiceFrame *pIMainServiceFrame,IServerUserItemSink *pIServerUserItem,ITimerEngine *pITimerEngine,IAndroidUserManager *pAndroidUserManager)=NULL;
	//初始化桌子框架
...
public:
	...
	//比赛进入消息
	virtual bool OnEventEnterMatch(DWORD dwSocketID ,VOID* pData,DWORD dwUserIP)=NULL;
