	...
	case IDI_ANDROID_INOUT:			//进出处理
		{
			...
			if ((bAllowAndroidAttend==true)||(bAllowAndroidSimulate==true))
			{
				...
				for (WORD i=0;i<m_wStockCount;i++)
				{
					...
					if((m_pGameServiceOption->wServerType==GAME_GENRE_MATCH)
						&& (m_AndroidParameter[i].dwServiceGender&ANDROID_SIMULATE)!=0
						&& (m_AndroidParameter[i].dwServiceGender&ANDROID_PASSIVITY)==0
						&& (m_AndroidParameter[i].dwServiceGender&ANDROID_INITIATIVE)==0)
					{
						continue;
					}
			...
			while (Position!=NULL)
			{
				...
				if (pAndroidUserItem->m_pIServerUserItem!=NULL)
				{
					...
					//比赛状态
					BYTE cbEnlistStatus=pIServerUserItem->GetUserEnlist();
					if (cbEnlistStatus!=MS_NULL) continue;
