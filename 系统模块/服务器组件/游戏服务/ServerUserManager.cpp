...
//用户日志
bool CServerUserItem::WriteUserLog(WORD wRequestID, VOID  * pData, DWORD dwDataSize)
{
	//发送状态
	ASSERT(m_pIServerUserItemSink!=NULL);
	if (m_pIServerUserItemSink!=NULL) m_pIServerUserItemSink->OnEventUserItemLog(this, wRequestID, pData, dwDataSize);
...
//真实玩家在线人数
DWORD CServerUserManager::GetRealUserItemCount()
{
	DWORD dwCount=0;
	CServerUserItem * pServerUserItem=NULL;
	for (INT_PTR i=0; i<m_UserItemArray.GetCount(); i++)
	{
		pServerUserItem=m_UserItemArray[i];
		if (NULL!=pServerUserItem && !pServerUserItem->IsAndroidUser())
		{
			dwCount++;
		}
	}
...
bool CServerUserItem::AddUserItem(const tagUserItem * pUserItem)
{
	if (NULL == pUserItem)
	{
		return false;
	...
	m_vUserItem.push_back(*pUserItem);

	return true;
}

tagUserItem * CServerUserItem::GetUserItems()
{
	return &(m_vUserItem[0]);
}

int CServerUserItem::GetUserItemCount()
{
	return (int)m_vUserItem.size();
}
