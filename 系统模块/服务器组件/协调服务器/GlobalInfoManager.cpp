...
//获取真实玩家数量
DWORD CGlobalServerItem::GetRealCount()
{
	DWORD dwRet = 0;
	...
	{
		//获取数据
		m_MapUserID.GetNextAssoc(Position,dwUserKey,pGlobalUserItem);
		if (pGlobalUserItem != NULL && !pGlobalUserItem->IsAndroid())
		{
			dwRet++;
		}
	}
...
bool CGlobalInfoManager::ActiveUserItem(tagGlobalUserInfo & GlobalUserInfo, WORD wServerID)
{
	...
	if (m_MapUserID.Lookup(GlobalUserInfo.dwUserID,pGlobalUserItem)==FALSE)
	{
		...
		pGlobalUserItem->m_cbAndroid=GlobalUserInfo.cbAndroid;
