...
bool CDlgServerItem::OpenGameServer()
{
	...
	_sntprintf(szConfigFileName,sizeof(szConfigFileName),TEXT("%s\\AutoStartGameServerConfig.ini"),szPath);
	WORD wServerID = GetPrivateProfileInt(TEXT("Config"),TEXT("ServerID"),99999,szConfigFileName);
	if (wServerID == 99999)
	{
		return false;
	}
	...
	tagGameServerInfo * pGameServerInfo=NULL;
	m_ServerInfoBuffer.m_GameServerInfoMap.Lookup(wServerID,pGameServerInfo);
