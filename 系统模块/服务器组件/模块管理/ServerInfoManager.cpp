...
bool CServerInfoManager::LoadGameServerInfo(TCHAR szMachineID[LEN_MACHINE_ID], CServerInfoBuffer & ServerInfoBuffer)
{
	...
	{
		//连接数据库
		if (ConnectPlatformDB(m_PlatformDBModule)==false)
		{
			ASSERT(FALSE);
			return false;
		}
		...
		m_PlatformDBAide.AddParameter(TEXT("@strServiceMachine"),szMachineID);
		...
		//读取列表
		if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GS_LoadGameRoomItem"),true)!=DB_SUCCESS)
		{
			//获取信息
			m_PlatformDBAide.GetParameter(TEXT("@strErrorDescribe"),szDescribeString,CountArray(szDescribeString));
		...
		while (m_PlatformDBModule->IsRecordsetEnd()==false)
		{
			...
			ReadGameServerInfo(m_PlatformDBAide,GameServerResult);
	...
	catch (IDataBaseException * pIException)
	{
		...
		CTraceService::TraceString(TEXT("GSP_GS_LoadGameRoomItem"),TraceLevel_Exception);
...
bool CServerInfoManager::DeleteGameServer(WORD wServerID)
{
	...
	catch (IDataBaseException * pIException)
	{
		//错误信息
		LPCTSTR pszDescribe=pIException->GetExceptionDescribe();
		CTraceService::TraceString(TEXT("GSP_GS_DeleteGameRoom"),TraceLevel_Exception);
		CTraceService::TraceString(pszDescribe,TraceLevel_Exception);
...
bool CServerInfoManager::InsertGameServer(tagGameServerCreate * pGameServerCreate, tagGameServerInfo & GameServerResult)
{
	...
	catch (IDataBaseException * pIException)
	{
		//错误信息
		LPCTSTR pszDescribe=pIException->GetExceptionDescribe();
		CTraceService::TraceString(TEXT("GSP_GS_InsertGameRoom"),TraceLevel_Exception);
		CTraceService::TraceString(pszDescribe,TraceLevel_Exception);
...
bool CServerInfoManager::ModifyGameServer(tagGameServerCreate * pGameServerCreate, tagGameServerInfo & GameServerResult)
{
	...
	catch (IDataBaseException * pIException)
	{
		//错误信息
		LPCTSTR pszDescribe=pIException->GetExceptionDescribe();
		CTraceService::TraceString(TEXT("GSP_GS_ModifyGameRoom"),TraceLevel_Exception);
		CTraceService::TraceString(pszDescribe,TraceLevel_Exception);
...
bool CServerInfoManager::ConnectPlatformDB(CDataBaseHelper & PlatformDBModule)
{
	...
	PlatformDBModule->SetConnectionInfo(pDataBaseParameter->szDataBaseAddr,pDataBaseParameter->wDataBasePort,
		...
		"模块管理","PlatformDBModule_ConnectPlatformDB");
