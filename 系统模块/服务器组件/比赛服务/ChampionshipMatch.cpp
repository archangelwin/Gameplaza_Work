...
bool CChampionshipMatch::OnEventGameStart(ITableFrame *pITableFrame, WORD wChairCount)
{
	...
	if(KillMatchTimer(IDI_CheakTabelStart,(WPARAM)pITableFrame)==false)ASSERT(FALSE);
	...
	case IDI_CHECK_MATCH_GROUP:
		{
			//打立出局模式
			static int timer=0;
			if(m_pMatchRule.enFirstRule==FirstRoundRule_Score&&timer++%5==0)//每5秒进来一次
			{
				//预赛状态
...
WORD CChampionshipMatch::SortUserRanking(tagMatchRanking MatchRanking[])
{
	INT_PTR nCount=m_OnMatchUserMap.GetCount();
	POSITION pos=m_OnMatchUserMap.GetStartPosition();
	nCount=0;
...
//空闲桌子拉空闲用户
void CChampionshipMatch::TabelRequestUser()
{
	//参赛用户不够 椅子数量就要终止比赛
	if (CheckMatchUser())
	{
	...
	{
		if(m_MatchTableArray[i]->pTableFrame->IsGameStarted()==false)
		{
	...
	for (INT_PTR i=0;i<FreeTableArray.GetCount();i++)
	{
		...
		if(nCurUserCount/m_wChairCount>0)
		{
			...
			{
				//随机用户插入该桌
				WORD randmer=rand()%(WORD)m_FreeUserArray.GetCount();
				//先起立在坐下 让该桌的人全部起立
...
void CChampionshipMatch::ContinueGame(ITableFrame *pITableFrame)
{
	...
	if (pITableFrameEx)
	{
		...
		for(int i=0;i<m_wChairCount;i++)
		{
			IServerUserItem* pUserItem=m_FreeUserArray[i];//pITableFrameEx->m_FrontUserArray[i];
...
void CChampionshipMatch::CheakTableStart(ITableFrame *pITableFrame)
{
	if(pITableFrame&&pITableFrame->IsGameStarted()==false)
	{
		...
		for (int i=0;i<m_wChairCount;i++)
		{
			...
			{
				m_pMatchSink->SendGameMessage(pUserItem, TEXT("由于本桌卡賽，请等待再次为您配桌！"),SMT_CHAT|SMT_TABLE_ROLL);
				pITableFrame->PerformStandUpAction(pUserItem);
				m_FreeUserArray.Add(pUserItem);
			}
		}
...
void CChampionshipMatch::LastMatchResult()
{
	if(m_pMatchRule.dwInitalScore==0)
	{
		...
		WORD wCount=SortUserRanking(pScore);
		for (WORD i=0;i<wCount;i++)
		{
