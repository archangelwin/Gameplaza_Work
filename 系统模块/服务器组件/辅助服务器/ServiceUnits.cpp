...
bool CServiceUnits::InitializeService()
{
	...
	if (m_TCPNetworkEngine->SetTCPNetworkEngineEvent(pIAttemperEngine,TEXT("辅助服务器"))==false) return false;
	...
	case UI_CORRESPOND_RESULT:		//协调结果
		{
			...
			LPCTSTR pszString=TEXT("注册成功");
			CTraceService::TraceString(pszString,TraceLevel_Normal);
