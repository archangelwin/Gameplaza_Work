...
#define PARAMETER_A 0.9996f
#define PARAMETER_B 1.18f
	...
	case GAME_SCENE_PLAY://游戏状态
		{
			...
			//修正剧情时和过海浪时出散鱼的问题
			if (!m_bNoFish && !m_bFishingOffSeason)
			{
...
bool CTableFrameSink::OnSubHitFish(IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize)
{
	...
	if (!pLotteryInfo->bCouldGet && PlayerInfo.nLotteryGift > 0)
	{
		...
		if (m_Config.nMinScore == 25 && PlayerInfo.nLotteryGift >= 10)
		{
		...
		else if (m_Config.nMinScore == 250 && PlayerInfo.nLotteryGift >= 10)
		{
			LotteryInfo.bCouldGet = true;
			bLotteryGift = true;
		...
		else if (m_Config.nMinScore == 2500 && PlayerInfo.nLotteryGift >= 50)
		{
			LotteryInfo.bCouldGet = true;
			bLotteryGift = true;
		...
		else if (m_Config.nMinScore == 25000 && PlayerInfo.nLotteryGift >= 100)
		{
			LotteryInfo.bCouldGet = true;
			bLotteryGift = true;
	...
	if (!pMobileMoney->bCouldGet)
	{
		if (PlayerInfo.wNewPlayerMobileMoney < 9)
		{
			...
			struct tm * ptm1 = localtime(&tMobileMoney);
			time_t tNow = time(NULL);
			struct tm * ptm2 = localtime(&tNow);
...
//printf("tm null\n");
	...
	if (m_pGameServiceOption->wServerID == 309)
	{
		__int64 nPassTime=0;
...
//printf("fUserLucky = %.2f\n", fUserLucky);
	...
	//	}
	//}
	float f1 = powf(PARAMETER_A, nCurrentPercent);
//printf("powf(PARAMETER_A, nCurrentPercent) %f\n", f1);
	float f2 = 1.f + (float)nStorage / m_Config.m_fAutoStockDiffMaxValue;
//printf("(float)nStorage %f m_Config.m_fAutoStockDiffMaxValue %f f2 %f\n", (float)nStorage, m_Config.m_fAutoStockDiffMaxValue, f2);
	float f3 = logfab(PARAMETER_B, f2);
//printf("logfab(PARAMETER_B, f2) %f\n", f3);
	fAutoStockDiffFactor = 1.f + f1 * f3;
//printf("fAutoStockDiffFactor=%.2f\n",fAutoStockDiffFactor);
	...
	if (nRandomPercent<nCurrentPercent)
	{
		...
		if (NULL!=pPlotInfo && !m_bFishingOffSeason)
		{
	...
	if (!PlayerInfo.bScoreFull && PlayerInfo.lUserAllScore >= m_pGameServiceOption->lMaxEnterScore && m_pGameServiceOption->wServerID == 309)
	{
//printf("standup\n");
...
//printf("PlayerInfo.wNewPlayerMobileMoney is %hu\n", PlayerInfo.wNewPlayerMobileMoney);
				SaveUserItem(pIServerUserItem,
					eUserItem_NewPlayerMobileMoney,
					eUserItemFlag_Heap|eUserItemFlag_GamePreLoad|eUserItemFlag_OverWrite,
					PlayerInfo.wNewPlayerMobileMoney,false);
			}
			//对于新版本，分享通过网站，话费由网站更新
			PlayerInfo.wMobileMoneyWill = pMobileMoney->wPrice;
			SaveUserItem(pIServerUserItem,
				eUserItem_MobileShare,
				eUserItemFlag_Heap|eUserItemFlag_GamePreLoad|eUserItemFlag_OverWrite,
				pMobileMoney->wPrice,false);
			//发送得奖消息
			CMD_S_MobileMoney MobileMoney = {0};
			MobileMoney.wMoneyCount = pMobileMoney->wPrice;
...
bool CTableFrameSink::OnSubFire(IServerUserItem * pIServerUserItem, float fAngle, DWORD dwBulletID, int nCount, BYTE cbAttrib)
{
	...
	//将子弹加入数组
	PlayerInfo.addBullet(dwBulletID,nChangeScore,PlayerInfo.bTryCannon,bHitMore,_GetTickCount64());
