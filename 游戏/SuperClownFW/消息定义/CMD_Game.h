...
#define SUB_S_SEND_USEITEM_RESLUT         125                           //转动结果(用道具)
#define SUB_S_USER_CANT_PLAY			  126                           //玩家分数低于要求不能玩的提示
...
struct CMD_S_GAME_READY
{
	...
	int				nItemNum;							//当前道具数量
	int				nFiveStarExtralReward;				//五星额外奖励
	int				nSixStarExtralReward;				//六星额外奖励
	int				nSevenStarExtralReward;				//七星额外奖励
	int				nEightStarExtralReward;				//八星额外奖励
};
//开始游戏
...
struct CMD_S_SEND_RESULT
{
	...
	bool            bLackChip;                          //是否不足下注
	LONGLONG        lUserScore;                         //用户当前分数
	LONGLONG        lCurrentChip;                       //当前下注数
	LONGLONG        lDefultChip;                        //游戏默认所需金币
};
...
struct CMD_S_CHANGE_BET
{
	...
	LONGLONG		 lUserScore;					   	 //玩家金币数量
	LONGLONG		 lDefultChip;					     //默认消耗金币量
	BYTE			 cbCanChange;						 //改变结果1.成功。2.玩家分数不够。3.不能小于默认值。
};
//停止转动
...
#define SUB_C_GO_GAME_ITEM              306               //开始命令(使用道具)
